...
export class GrCarousel extends GrObject {
  ...
  constructor(params = {}) {
    ...
    for (let i = 0; i < num_poles; i++) {
      ...
      let horse = loader.loadAsync("../libs/Horse.obj");
      horse.then(function(horse) {
        horse.position.set(0, -0.5+0.5*Math.sin(4*Math.PI*i/num_poles), 0);
        horse.scale.set(0.0009, 0.0009, 0.0009);
  ...
  stepWorld(delta, timeOfDay) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (typeof horse !== 'undefined') {
        horse.position.y = -0.5+0.5*Math.sin(-3*this.rotateAngle + 4*Math.PI*i/10);
      }
    }
  }
}
export class Wolf extends GrObject {
  constructor() {
    ...
    super(`Wolf-${carouselObCtr++}`, obj);
    let wolf = loader.loadAsync("../libs/Wolf.obj");
  ...
  stepWorld(ms, daytime) {
    ...
    this.obj.rotation.x += (0.01 * ms) / 16;
    this.obj.rotation.y += (0.01 * ms) / 16;
  }
}
...
export class GrJumpRide extends GrObject {
  ...
  constructor(params = {}) {
    ...
    // note that we have to make the Object3D before we can call
    // super and we have to call super before we can use this
    super(`JumpRide-${carouselObCtr++}`, carousel);
    this.whole_ob = carousel;
    this.platform = platform_group;
    this.jump = cab;
    this.position = 0;
  ...
  stepWorld(delta, timeOfDay) {
    ...
    console.log(Math.floor(this.position)) 
