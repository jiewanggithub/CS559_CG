...
function twoQuarterCircles(u) {
  ...
  if (u < 0.5)
    return [
      50 + 50 * Math.cos(pu), // the center is at 50,50, y = sin(theta)*radius
      50 * Math.sin(pu),  // x = cos(theta)*radius
      -50 * Math.sin(pu),
      50 * Math.cos(pu)
    ];
  else
...
function twoLines(u) {
  if (u < 0.5) return [20 + 80 * u, 200 * u, 80, 200];
  else return [20 + 80 * u, 100 - 200 * (u - 0.5), 80, -200];
}
...
export function functionGallery(context, t, tangentScale) {
  ...
  // since the moveto/drawto will connect them
  context.beginPath();
  context.moveTo(0, 0);
