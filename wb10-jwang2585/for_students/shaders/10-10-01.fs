...
void main(void){
    vec2 m =  mouse;
	m *= rot(-time);
	vec3  cPos = vec3(1.0 * 400.*sin(0.1*time), 1. * 60.*cos(0.1*time), 2.0 - 3.*time*2.);   // movement
	...
	for(int i = 0; i < 200; i++){
		...
		rLen += distance+1.*sin(sin(2.*time+rPos.y));
		rPos = cPos + ray * rLen;
	}
	// the fog
	float fog = 1.0 - pow(20./rLen, FOG_STRENGTH);
	vec3 c = vec3(0.0);
	// hitting
